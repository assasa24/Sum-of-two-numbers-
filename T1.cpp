...
void checkInput(vector<int>& vec, int x, int size) {
    ...
    //TODO :need to check if size in general is equal to size entered
...
/*
            if (Curr.y != 0){
                if(Curr.x == arr[Curr.n - 1] + Curr.y){
                    cout << arr[Curr.n-1] <<" "<< Curr.y << endl;
                }
                Curr.line = AFTER_SECOND;
                S.Push(Curr);
...
/*
 if (returnFromRecursion)
        ...
        if (Curr.line == START){
            if (Curr.n == 1){
                if (Curr.y != 0 && Curr.x == arr[0] + Curr.y)
                    cout << arr[0] <<" "<< Curr.y << endl;
                returnFromRecursion = true;
            }
            else{
